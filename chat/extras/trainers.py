    ...
    def train(self, *corpus_paths):
        ...
        for corpus_path in corpus_paths:
            ...
            for corpus_count, corpus in enumerate(corpora):
                # Adiciona as categorias do corpus
                for categoria in corpus.categories:
                    Categoria = self.storage.get_model('categoria')
                    cat = Categoria(name=categoria)
                    cat.save()
                ...
                for conversation_count, conversation in enumerate(corpus):
                    ...
                    for text in conversation:
                        ...
                        # Add categoria ao statement
                        Stat = self.storage.get_model('statement')
                        statement_last = Stat.objects.all().last()
                        statement_last.categorias.add(cat)

                # Stat = self.storage.get_model('statement')
                # statement_list = Stat.objects.all().last()
                # for statement in statement_list:
                #     statement.categorias.add(cat)
