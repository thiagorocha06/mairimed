...
import logging
from chat.extras.storage import StorageAdapter
from chat.extras.input import InputAdapter
from chat.extras.output import OutputAdapter
from chat.extras import utils
...
class ChatBot(object):
    ...
    def __init__(self, name, **kwargs):
        ...
        storage_adapter = kwargs.get('storage_adapter', 'chat.extras.storage.SQLStorageAdapter')

        # These are logic adapters that are required for normal operation
        system_logic_adapters = kwargs.get('system_logic_adapters', (
            'chat.extras.logic.NoKnowledgeAdapter',
        ))

        logic_adapters = kwargs.get('logic_adapters', [
            'chat.extras.logic.BestMatch'
        ])

        input_adapter = kwargs.get('input_adapter', 'chat.extras.input.VariableInputTypeAdapter')

        output_adapter = kwargs.get('output_adapter', 'chat.extras.output.OutputAdapter')
        ...
        preprocessors = kwargs.get(
            'preprocessors', [
                'chat.extras.preprocessors.clean_whitespace'
            ]
        )
        ...
        trainer = kwargs.get('trainer', 'chat.extras.trainers.Trainer')
