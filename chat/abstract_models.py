from chat.extras.conversation import StatementMixin
...
if hasattr(settings, 'CHATTERBOT'):
    ...
    CATEGORIA_MODEL = settings.CHATTERBOT.get(
        'categoria_model',
        CATEGORIA_MODEL
    )
...
class AbstractBaseStatement(models.Model, StatementMixin):
    ...
    categorias = models.ManyToManyField(CATEGORIA_MODEL,
    ...
    def add_categorias(self, categorias):
        """
        Add a list of strings to the statement as tags.
        (Overrides the method from StatementMixin)
        """
        for categoria in categorias:
            self.categorias.create(
                name=categoria
            )
...
class AbstractBaseCategoria(models.Model):
    """
    The abstract base tag allows other models to
    be created using the attributes that exist on the
    default models.
